...
from typing import Tuple, Dict, List, Deque, Optional, Iterable, Union, Type
from dataclasses import dataclass, field, replace
from dataclasses_json import config, dataclass_json, DataClassJsonMixin
from bgameb.base import Base, Base_, Component_
from bgameb.items import Card, Dice, Step, BaseItem, Dice_, Card_, Step_, BaseItem_
from bgameb.errors import ArrangeIndexError, ComponentClassError
from bgameb.constraints import ITEMS


Item = Union[Card_, Dice_, Step_]
...
@dataclass(repr=False)
class Bag_(BaseTool_, DataClassJsonMixin):
    """Bag object
    """
    i: Component_[str, Item] = field(default_factory=dict)
    ...
    def deal(self, items: Optional[List[str]] = None) -> 'Bag':
        ...
        if not items:
            for stuff in self.i.values():
                if issubclass(type(stuff), BaseItem_):
                    self.append(stuff)
        else:
            for id in items:
    ...
    def add(self, stuff: Type[Item]) -> None:
        """Add stuff to component

        Args:
            stuff (Type[Item]): game stuff
        """
...
@dataclass(repr=False)
class Shaker_(BaseTool_, DataClassJsonMixin):
    """Create shaker for roll dices or flip coins
    """
    current: List[Dice_] = field(
        default_factory=list,
        metadata=config(exclude=lambda x: True),  # type: ignore
        repr=False,
        )
    i: Component_[str, Dice_] = field(default_factory=dict)
    ...
    def deal(self, items: Optional[List[str]] = None) -> 'Shaker':
        ...
        if not items:
            for stuff in self.i.values():
                if issubclass(type(stuff), Dice_):
                    self.append(stuff)
        else:
            for id in items:
                if id in self.i.keys():
    ...
    def add(self, stuff: Type[Dice_]) -> None:
        """Add stuff to component

        Args:
            stuff (Type[Dice_]): game stuff
        """
        if isinstance(stuff.__class__, Dice_) or issubclass(stuff.__class__, Dice_):
            self.i._update(stuff)
            self._logger.info(f'"{stuff.id}" is added to i".')
        else:
...
@dataclass(repr=False)
class Deck_(BaseTool_, DataClassJsonMixin):
    """Deck object
    ...
    """
    current: Deque[Card_] = field(
        default_factory=deque,
        metadata=config(exclude=lambda x: True),  # type: ignore
        repr=False,
        )
    i: Component_[str, Card_] = field(default_factory=dict)
    ...
    def deal(self, items: Optional[List[str]] = None) -> 'Deck':
        ...
        if not items:
            for stuff in self.i.values():
                if issubclass(type(stuff), Card_):
    ...
    def add(self, stuff: Type[Card_]) -> None:
        """Add stuff to component

        Args:
            stuff (Type[Card_]): game stuff
        """
        if isinstance(stuff.__class__, Card_) or issubclass(stuff.__class__, Card_):
            self.i._update(stuff)
            self._logger.info(f'"{stuff.id}" is added to i".')
        else:
            raise ComponentClassError(stuff, self._logger)
...
@dataclass(repr=False)
class Steps_(BaseTool_, DataClassJsonMixin):
    ...
    """
    current: List[Tuple[int, Step_]] = field(
        default_factory=list,
        metadata=config(exclude=lambda x: True),  # type: ignore
        repr=False,
            )
    ...
    i: Component_[str, Step_] = field(default_factory=dict)
    ...
    def deal(
        ...
        if not items:
            for stuff in self.i.values():
                if issubclass(type(stuff), Step_):
                    self.push(stuff)
        else:
            for id in items:
                if id in self.i.keys():
    ...
    def add(self, stuff: Type[Step_]) -> None:
        """Add stuff to component

        Args:
            stuff (Type[Step_]): game stuff
        """
        if isinstance(stuff.__class__, Step_) or issubclass(stuff.__class__, Step_):
            self.i._update(stuff)
            self._logger.info(f'"{stuff.id}" is added to i".')
        else:
            raise ComponentClassError(stuff, self._logger)
...
class Shaker(BaseTool):
    ...
    def deal(self, items: Optional[List[str]] = None) -> 'Shaker':
        ...
        if not items:
            ...
            for comp in self:
                if issubclass(type(self[comp]), Dice):
                    self.append(self[comp])
            ...
            for id_ in items:
                for comp in self:
                    if isinstance(self[comp], Dice) \
                            and self[comp].id == id_:
                        self.append(self[comp])
                        break

        self._logger.debug(f'Is deal dices: {self.current}')
        return self
...
class Steps(BaseTool):
    ...
    def deal(
        ...
        if not items:
            ...
            for name in items:
                if name != 'last' and name in names:
                    if not used.get(name):
                        for comp in self:
