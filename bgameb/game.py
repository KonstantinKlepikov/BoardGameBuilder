...
from bgameb.types import (
    COMPONENTS, COMPONENTS_TYPES, NONSTUFF, NONSTUFF_TYPES,
    STUFF, STUFF_TYPES, SPETIAL_STUFF
    )
        ...
        if type_ in COMPONENTS.keys():
            if not target:
                ...
                self._logger.info(f'{name} is added to game.')
                    ...
                    or type_ in SPETIAL_STUFF.keys()):
    ...
    def copy(
        ...
        **kwargs
        ...
        if source in self.keys() \
                ...
                and (self[target]._type in NONSTUFF.keys()
                or self[target]._type in SPETIAL_STUFF.keys()):
