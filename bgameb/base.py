...
import re
import string
from typing import List, Optional, Iterator, Dict, Any, Type, TypeVar, NoReturn
...
class Base_:
    ...
    def __post_init__(self) -> None:
        ...
        if 'BaseGame' in [cl.__name__ for cl in self.__class__.__mro__]:
            self._logger.info('===========NEW GAME============') # NOTE: ambiculous
        self._logger.info(
...
V = TypeVar('V', bound=Type[Base_])


class Component_(Mapping[K, V]):
    """Component mapping
    """
    ...
    def __delattr__(self, attr: K) -> None:
        try:
    ...
    def _update(
        ...
        comp = stuff.__class__(**stuff.to_dict())
    ...
    def get_names(self) -> List[str]:
        ...
        """
        return [name for name in self._inclusion]
    ...
    def by_id(self, id: str) -> Optional[V]:
        ...
        """
        for comp in self._inclusion:
            if issubclass(comp.__class__,  Base_) and self[comp].get('id') == id:
                return self[comp]
        return None
