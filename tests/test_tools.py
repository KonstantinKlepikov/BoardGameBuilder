...
class TestShaker:
    ...
    @pytest.fixture
    def comp(self) -> Components[str, Dice]:
        return Components[str, Dice](
            dice=Dice(id='dice', count=5),
            dice_nice=Dice(id='dice_nice', count=5)
                )

    @pytest.fixture
    def dealt_obj_(self, obj_: Shaker, comp: Components[str, Dice]) -> Shaker:
        obj_.deal(comp)
        return obj_
...
class TestDeck:
    ...
    @pytest.fixture
    def comp(self) -> Components[str, Card]:
        return Components[str, Card](
            card=Card(id='card', count=5),
            card_nice=Card(id='Card_nice', count=5)
                )

    @pytest.fixture
    def dealt_obj_(self, obj_: Deck, comp: Components[str, Card]) -> Deck:
        obj_.deal(comp)
        return obj_
    ...
    def test_deck_deal(self, obj_: Deck, comp: Components[str, Card]) -> None:
        """Test deck deal()
        """
    ...
    def test_deck_deal_from_list(self, obj_: Deck, comp: Components[str, Card]) -> None:
        """Test deck deal() from items
        """
        items = ['card', 'card', 'card', 'Card_nice']
        result = obj_.deal(comp, items).current
    ...
    def test_search(self, obj_: Deck, comp: Components[str, Card]) -> None:
        """Test deck search() one or many or no one cards
        """
        comp.card.count = 2
...
class TestSteps:
    ...
    @pytest.fixture
    def comp(self) -> Components[str, Step]:
        return Components[str, Step](
            step1=Step(id='step1', priority=1),
            astep=Step(id='asteP', priority=2)
                )

    @pytest.fixture
    def dealt_obj_(self, obj_: Steps, comp: Components[str, Step]) -> Steps:
        obj_.deal(comp)
        return obj_
